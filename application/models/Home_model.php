...
class Home_model extends CI_Model
{
    ...
    public function get_student_data($student_reg_no, $receipt_no)
    {
        $slt_ary = array(
            'a.*',
        );
        $this->db->select($slt_ary);
        $this->db->from('students as a');
        $this->db->join('receipt as b', 'a.student_id=b.fk_student_id', 'left');
        $this->db->where(array(
            'a.status' => '1',
            'a.archive_status' => '1',
            'a.student_barcode' => $student_reg_no,
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->where(array(
                ...
                'student_dob' => date('Y-m-d', strtotime($receipt_no)),
            ));
            $query = $this->db->get();
            if ($query->num_rows() > 0) {
                return $query->row_array();
