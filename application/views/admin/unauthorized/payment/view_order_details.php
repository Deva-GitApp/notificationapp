...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active"><a href="<?php echo base_url('admin/payment/view_order_details'); ?>">Payment Order Details</a></li>
        </ol>
    ...
    <div class="ajax_order_detial_result">
...
//        "columnDefs": [{orderable: false, targets: [4, 7, 10, 8]}],
        ...
        "ajax": {
            "url": "<?php echo base_url('admin/payment/ajax_order_details_datatable') ?>",
            "dataType": "json",
            "type": "POST",
    ...
    $('body').on('click', '.get_raz_order_details', function () {
        var elem = $(this);
        var orderId = $(this).attr('data-orderid');
        $.ajax({
            method: "POST",
            url: "<?php echo base_url('admin/payment'); ?>/get_raz_order_details",
            data: {data: orderId},
            // dataType: 'json',
            success: function (msg) {
                $(elem).addClass('get_raz_order_details').html('View Order Details');
                if (msg != '') {
                    $('.ajax_order_detial_result').html('').html(msg);
                } else {
                    $.confirm({
                        content: 'No datas available for ' + orderId,
                        icon: 'fa fa-warning',
                        theme: 'modern',
                        closeIcon: false,
                        animation: 'scale',
                ...
                $(elem).removeClass('get_raz_order_details').html('Please wait');
            }
        });
    });
</script>
